...
> **诊断时的一个陷阱（实测踩过）**：统计字面量 `##` / `**` / ``` 之前，**必须先把
> `<pre>` / `<code>` 区域挖空**。在代码块里合法出现 ``` 或 `**` 是渲染**正常**的证据——
> 示例代码、以及**讲 Markdown 语法本身的文章**都会这样。不做区分就会把正常文章误判成
> 未渲染（实测：一篇正文渲染完全正常的文章被报出「字面 ``` × 1、`**` × 3」，
> 逐个查下来全在代码块里）。`check_render.py` 已内置这一步，输出里会说明排除了代码区域。
